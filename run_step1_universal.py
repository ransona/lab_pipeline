...
sys.path.append('/home/adamranson/code/preprocess_py')
...
def _build_command(command_filename, user_id, exp_id, runs2p, rundlc, runfitpupil):
    return (
        'preprocess_step1_universal.run_preprocess_step1_universal('
...
def _notify_queue_position(user_id, exp_label, queue_path):
    ...
    try:
        matrix_msg.main(user_id, 'Added ' + exp_label + ' to queue in position ' + str(len(files)))
    except Exception:
...
def run_step1_batch_universal(step1_config):
    ...
    _, _, _, _, first_exp_raw = organise_paths.find_paths(user_id, first_exp)
